...
vim.api.nvim_create_autocmd('LspAttach', {
  callback = function(event)
    ...
    -- Ignore 'unused variable' warnings from bashls for .env files
    if client and client.name == 'bashls' and vim.fn.expand('%:t'):match '^%.env' then
      -- Intercept diagnostics from the LSP
      client.handlers['textDocument/publishDiagnostics'] = function(err, result, ctx, config)
        -- Only filter if the file is a .env file
        local filename = vim.fn.fnamemodify(vim.uri_to_fname(result.uri), ':t')
        if filename:match '^%.env' then
          result.diagnostics = vim.tbl_filter(function(diagnostic)
            -- Filter out SC2034 (unused variable)
            return diagnostic.code ~= 'SC2034'
          end, result.diagnostics)
        end

        -- Call the default handler to display the remaining diagnostics
...
require('lazy').setup({
  ...
  { -- Collection of various small independent plugins/modules
    ...
    config = function()
      ...
      require 'custom.configs.mini-statusline'
      -- ... and there is more!
